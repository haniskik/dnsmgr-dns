...
| **固定业务域名** | 可配置一个固定二级域名（如 `pay.example.com`），每次轮换执行后，其 **CNAME 记录自动更新**为当日最新随机域名，业务方始终访问固定域名即可。 |
...
- 主域名：`example.com`
- A 槽解析：原主机记录 `cs1`，轮换后可能变为 `cs8a247d`、`cs2510b7` 等随机值。
- B 槽解析：原主机记录 `cs2`，轮换后可能变为 `cs446a39`、`csa86d44` 等随机值。
- 固定业务域名：如 `pay.example.com`（CNAME），每次执行轮换后自动指向**本次刚生成**的随机域名（如 `cs2510b7.example.com`）。

...
本仓库在 [原版 dnsmgr](https://github.com/netcccyun/dnsmgr) 基础上新增：**每日 A/B 槽随机二级域名轮换**、**容灾联动**、**固定业务域名 CNAME 自动指向最新随机域名**、**后台 A/B 轮换配置页**。  
详细功能说明与使用步骤见：[**FEATURES_AB_ROTATE.md**](./FEATURES_AB_ROTATE.md)。

## 功能特性

- **【二开】A/B 轮换**：每日定时将两条解析（A/B 槽）的主机名改为随机二级域名，并与容灾切换、固定业务域名 CNAME 联动；
- **【二开】固定域名 CNAME**：可配置一个固定二级域名，轮换执行后其 CNAME 自动指向当日最新随机域名；
